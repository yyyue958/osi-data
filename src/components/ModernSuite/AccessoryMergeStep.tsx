  ...
  const runMergePipeline = () => {
    ...
    const newLogs: PipelineExecutionLog[] = [
      ...
      },
      {
        timestamp: new Date().toLocaleTimeString(),
        level: 'info',
        message: `Loading Knee Procedures dataset from '${config.kneeProceduresFileName}' (Row 3 header)...`
      }
    ];

    setTimeout(() => {
      // Filter Knee procedures by selected Ship Years
      const filteredKnee: AccessoryOrderRow[] = [];
      kneeProceduresData.forEach(row => {
        const shipYear = typeof row['Ship Year'] === 'number' 
          ? row['Ship Year'] 
          : parseInt(String(row['Ship Year']), 10);

        if (config.targetYears.includes(shipYear)) {
          // Align columns: Matl Availability Date -> Billing Date
          filteredKnee.push({
      ...
      const statsObj: PipelineStats = {
        ...
        droppedRows: kneeProceduresData.length - filteredKnee.length,
        warningsCount: 0,
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Left: Datasets and Year Alignment (7 cols) */}
        <div className="lg:col-span-7 space-y-6">
          
          {/* Card 1: Dual Input Files */}
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            ...
            <div className="border border-slate-200 bg-slate-50/70 rounded-lg p-3.5 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <p className="text-xs font-bold text-slate-900">2020-2022 Knee Procedures.xlsx</p>
                    <span className="text-[10px] bg-slate-100 text-slate-700 border border-slate-300 px-1.5 py-0.5 rounded font-mono">
                      Sheet: All accessory installs - clean
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 font-mono">
                    Headers on Row 3 • {kneeProceduresData.length} total rows
                  </p>
                </div>
              </div>

              <button
                onClick={() => onOpenPreview(kneeProceduresData, 'Knee Procedures Dataset Preview', 'knee_procedures.xlsx')}
                className="text-xs text-slate-700 hover:text-slate-900 font-semibold flex items-center gap-1 bg-white hover:bg-slate-100 px-2.5 py-1.5 rounded-md border border-slate-200 transition-colors"
              >
                <Eye className="w-3.5 h-3.5 text-slate-500" />
                <span>Preview</span>
            ...
            <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg flex items-center gap-3 text-xs text-slate-700">
              ...
              <span className="text-[11px] text-slate-500 ml-auto">
                (Auto-aligned &amp; ShipTo PostalCode included)
              </span>
            </div>

          </div>

          {/* Card 2: Interactive Year Chips & Safety Mode */}
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 rounded-md bg-emerald-100 text-emerald-800 flex items-center justify-center font-bold text-xs">
            ...
            {/* Safe Versioning Setting */}
            <div className="pt-2 border-t border-slate-100">
              <label className="text-xs font-bold text-slate-700 block mb-2">
                Data Persistence Safety Mode
              </label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  onClick={() => setSaveMode('safe-version')}
                  className={`p-3 rounded-lg border text-left text-xs transition-all ${
                    saveMode === 'safe-version'
                ...
                >
                  <p className="font-bold flex items-center gap-1.5 text-slate-900">
                    ...
                    <span>Safe Timestamped Copy (Recommended)</span>
                  </p>
                  <p className="text-[11px] text-slate-500 mt-1">
                    Prevents accidental corruption if re-run.
                  </p>
                </button>
                ...
                >
                  <p className="font-bold text-slate-900">
                    In-Place Append (Legacy Behavior)
                  </p>
                  <p className="text-[11px] text-slate-500 mt-1">
                    Updates clean file with backup copy created.
                  </p>
                </button>
              </div>
            </div>
        ...
        {/* Right: Actions, Metrics, Logs (5 cols) */}
