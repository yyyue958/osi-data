...
import { PipelineExecutionLog, PipelineStats } from '../../types';
import { Play, FileSpreadsheet, Download, MapPin, Sparkles, Upload } from 'lucide-react';
...
export const InstalledStateCleanerStep: React.FC = () => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLogs([{ timestamp: new Date().toLocaleTimeString(), level: 'info', message: `Selected file: ${file.name}. Ready to send to Python backend.` }]);
  };
  ...
  const runStateCleaning = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.detail || 'Server processing failed');
      ...
      setLogs(prev => [
        ...prev,
        { timestamp: new Date().toLocaleTimeString(), level: 'error', message: `❌ API ERROR: Ensure your Python backend (main.py) is running on port 8000.\nDetails: ${String(err)}` }
      ]);
    } finally {
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">Installed Base State Cleaning (Backend)</h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">Bypasses browser memory limits by streaming heavy files directly to the Python Pandas backend.</p>
        </div>
        <div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
          <MapPin className="w-4 h-4 text-emerald-600" />
          <span className="text-xs text-slate-700 font-medium">Backend Dictionary Active</span>
        </div>
      </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            ...
            <div className="border border-slate-200 bg-slate-50/70 hover:border-emerald-400 rounded-lg p-3.5 transition-colors relative group">
              ...
              <div className="mt-2.5 flex items-center gap-2">
                <label className="cursor-pointer text-[11px] font-semibold text-white hover:bg-emerald-700 flex items-center gap-1 bg-emerald-600 px-3 py-1.5 rounded-md shadow-sm transition-colors">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            ...
            <button
              ...
              className={`w-full py-3.5 px-4 rounded-md font-bold text-sm flex items-center justify-center gap-2 transition-all duration-150 shadow-sm ${
                isProcessing || !selectedFile ? 'bg-slate-400 text-white cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-700/20'
              }`}
            >
              {isProcessing ? (
                <>
          ...
          {stats && downloadUrl && (
            <div className="bg-white border border-emerald-300 rounded-xl p-5 shadow-md space-y-4 animate-in zoom-in-95 duration-200">
              ...
              >
                ...
                <span>Download Output from Server</span>
              </button>
            </div>
          )}
          ...
          <div className="bg-[#0f172a] border border-slate-800 rounded-xl p-4 font-mono text-xs space-y-2 shadow-sm">
            <div className="flex items-center justify-between text-slate-400 border-b border-slate-800 pb-2">
              <span className="font-bold text-[11px] uppercase tracking-wider text-slate-300">Server Activity Stream</span>
            </div>
