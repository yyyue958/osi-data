...
import { Play, FileText, Download, MapPin, Sparkles, Upload, Eye, AlertTriangle } from 'lucide-react';
...
export const InstalledStateCleanerStep: React.FC<InstalledStateCleanerStepProps> = ({ onOpenPreview }) => {
  // Uses active memory (useState), bypassing localStorage quota limits entirely
  ...
  const runStateCleaning = () => {
    ...
    const newLogs: PipelineExecutionLog[] = [
      { timestamp: new Date().toLocaleTimeString(), level: 'info', message: `Standardizing states using Pandas logic...` }
    ];
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-1">
            ...
            <span>•</span>
            <span>Geographic Standardization (CSV)</span>
          </div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">Installed Base CSV Cleaner</h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">Processes massive CSV datasets entirely in the browser memory.</p>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="font-bold text-slate-900 text-sm">Upload Raw CSV</h3>
              </div>
            </div>
